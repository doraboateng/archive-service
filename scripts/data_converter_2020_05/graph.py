...
def get_transliteration_uids(client, node_type, code, transliterations):
    ...
    for tr in transliterations:
        ...
        uid = get_uid_from_response(upsert_bak(
            ...
            'Transliteration',
            'hash',
            get_hash(node_type, code, tr.get('value')),
            tr,
        ))
...
def load_expressions(client, expressions):
    ...
    for expression in expressions:
        if bool(skip_titles & set([t.get('value') for t in expression.get('titles')])):
            continue
        ...
        for tr in expression.get('literal_translation'):
            add_transliteration_po_pair('literalTranslation', tr, po_pairs)
        
        for tr in expression.get('practical_translation'):
            add_transliteration_po_pair('practicalTranslation', tr, po_pairs)
        
        for tr in expression.get('meaning'):
            add_transliteration_po_pair('meaning', tr, po_pairs)
    ...
    check_response = client.txn(read_only=True).query(
        """{
            expressions(func: type(Expression)) {
                Expression.uuid
                Expression.practicalTranslation@.
    ...
    for ex in check_json['expressions']:
        ...
        translation = u''.join([ex.get('Expression.practicalTranslation@.')])
        print(f' - {ex.get("Expression.uuid")[:6]}: {transliterations} == {translation}')
